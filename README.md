...
The workflow has test job that will check if action will result in success if so; it does a deploy push to s3 followed by a deploy to ec2 instance [for stake of not going overboard I didnt make green blue deploy with load balancer]. A local file is created with terraform.tf info for s3 but a terrform run will have to be done to write file.
